...
def needle_reconstruction_ref( left_img, left_ref, left_filename, right_img, right_ref,right_filename,  stereo_params,
                               bor_l=None, bor_r=None,
                               roi_l=(), roi_r=(),
    ...
    # cv.imwrite('check_sub.png', imgs_ret[ 'sub' ])
    # cv.imwrite('check_seg-init.png', imgs_ret[ 'seg-init' ])
    ...
    #mask
    left_mask = cv.polylines( np.zeros(left_rect_full.shape),
                                         [ pts_l_match.reshape( -1, 1, 2 ).round().astype( np.int32 ) ], False,
                                         (255, 0, 0), 3 )
    right_mask = cv.polylines( np.zeros(right_rect_full.shape),
                                          [ pts_r_match.reshape( -1, 1, 2 ).round().astype( np.int32 ) ], False,
                                          (255, 0, 0), 3 )
    ...
    ### Revert remap to obtain segmentation for original mask
    # 1. Stack map_l and map_r to be (h, w, 2) instead of tuple (h,w) of size 2
    # 2. call invert_map on stacked map_l and map_r
    # 3. Apply remap

    map_l_new=np.stack((map_l[0], map_l[1]),axis=2)
    map_r_new=np.stack((map_r[0], map_r[1]),axis=2)
    ...
    inv_map_l_new = invert_map(map_l_new)
    inv_map_r_new = invert_map(map_r_new)
    # print("--- %s seconds ---" % (time.time() - start_time))

    inv_map_l_new = (inv_map_l_new[:,:,0],inv_map_l_new[:,:,1])
    inv_map_r_new = (inv_map_r_new[:,:,0],inv_map_r_new[:,:,1])

    left_mask_new = cv.remap( left_mask, inv_map_l_new[ 0 ], inv_map_l_new[ 1 ], cv.INTER_LINEAR )
    right_mask_new = cv.remap( right_mask, inv_map_r_new[ 0 ], inv_map_r_new[ 1 ], cv.INTER_LINEAR )

    if not os.path.exists('countor_match'):
        os.makedirs('countor_match')
    cv.imwrite("countor_match/"+left_filename+'.png', imgs_ret[ 'contours-match' ])

    # if not os.path.exists('imgs'):
    #     os.makedirs('imgs')
    # # cv.imwrite( "imgs/"+left_filename+'_left.png', left_rect_full)
    # # cv.imwrite( "imgs/"+right_filename+'_right.png', right_rect_full)
    # cv.imwrite( "imgs/"+left_filename+'_left.png', left_img)
    # cv.imwrite( "imgs/"+right_filename+'_right.png', right_img)
 
    left_gray=cv.cvtColor( np.float32(left_mask_new), cv.COLOR_BGR2GRAY )
    right_gray=cv.cvtColor(  np.float32(right_mask_new), cv.COLOR_BGR2GRAY )
    left_mask = cv.threshold(left_gray, 15, 255, cv.THRESH_BINARY)[1]  
    right_mask = cv.threshold(right_gray, 15, 255, cv.THRESH_BINARY)[1]  
    
    if not os.path.exists('masks'):
        os.makedirs('masks')
    cv.imwrite( "masks/"+left_filename+'_left_mask.png', left_mask)
    cv.imwrite( "masks/"+right_filename+'_right_mask.png', right_mask)
